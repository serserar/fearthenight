...
void renderbackgroundview(int w, int h, const char *caption, Texture *mapshot, const char *mapname, const char *mapinfo)
{
    ...
    settexture((maxtexsize ? min(maxtexsize, hwtexsize) : hwtexsize) >= 1024 && (hudw > 1280 || hudh > 800) ? "<premul>media/interface/logo_1024.png" : "<premul>media/interface/logo_512.png", 3);
    bgquad(lx, ly, lw, lh);
...
bool disablemousegrab = false;
void inputgrab(bool on)
{
    if(on && !disablemousegrab)
    {
...
void setupscreen()
{
    ...
    screen = SDL_CreateWindow("UndeadHunter", winx, winy, winw, winh, SDL_WINDOW_OPENGL | SDL_WINDOW_SHOWN | SDL_WINDOW_INPUT_FOCUS | SDL_WINDOW_MOUSE_FOCUS | flags);
...
void stackdumper(unsigned int type, EXCEPTION_POINTERS *ep)
{
    ...
    char out[512];
    formatstring(out, "UndeadHunter Win32 Exception: 0x%x [0x%x]\n\n", er->ExceptionCode, er->ExceptionCode==EXCEPTION_ACCESS_VIOLATION ? er->ExceptionInformation[1] : -1);
